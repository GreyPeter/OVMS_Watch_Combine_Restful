	...
	var body: some View {
		...
		NavigationStack {
			GeometryReader { watchGeo in
							...
							.overlay(
								VStack {
									if carMode == .charging {
										...
										Text("\(model.charge.estimatedrange)\(model.charge.units)")
											.fontWeight(.bold)
											.foregroundColor(Color.white)
									} else {
				...
				.navigationTitle(model.getVehicleID())
				//.navigationBarTitleDisplayMode(.inline)
				.toolbar {
					ToolbarItem(placement: .topBarLeading) {
						Button(action: {print("Gear Pressed")}, label: {
							Image(systemName: "gear")
						})
					}
					ToolbarItem(placement: .topBarTrailing){
						Image(systemName: carMode.image).foregroundColor(carMode.color)
					}
				}
